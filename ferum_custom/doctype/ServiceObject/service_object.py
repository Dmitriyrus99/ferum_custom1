...
class ServiceObject(Document):
    def validate(self):
        ...
        if self.is_new() or self.has_changed("object_name"):
            if frappe.db.exists("ServiceObject", {"object_name": self.object_name}) and \
               frappe.db.get_value("ServiceObject", {"object_name": self.object_name}, "name") != self.name:
    ...
    def on_trash(self):
        ...
        active_projects = frappe.get_all(
            "ServiceProject",
            filters={
                "objects.service_object": self.name,
                "status": ["in", ["Planned", "Active"]]
            },
            pluck="name"
        )
        if active_projects:
            frappe.throw(f"Cannot delete Service Object. It is linked to active Service Projects: {', '.join(active_projects)}")
        ...
        if active_requests:
            frappe.throw(f"Cannot delete Service Object. It is linked to active Service Requests: {', '.join(active_requests)}")

	...
	try:
		...
		if "Administrator" in user_roles or "Accountant" in user_roles or "Office Manager" in user_roles:
			...
			if invoice.get("project") not in managed_projects:
				raise HTTPException(
					status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to view this invoice."
				)
		elif "Client" in user_roles:
			customer_id = current_user.get("customer_id")
...
async def create_invoice(
	invoice_data: dict,
	current_user: Annotated[str, Depends(has_role(["Project Manager", "Administrator", "Office Manager"]))],
):
	try:
...
async def update_invoice_status(
	invoice_name: str,
	status_data: dict,
	current_user: Annotated[str, Depends(has_role(["Administrator", "Accountant"]))],
):
	try:

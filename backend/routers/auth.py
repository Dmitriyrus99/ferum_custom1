...
def authenticate_user(username: str, password: str):
    ...
    if username == "admin" and password == "admin": # Replace with actual ERPNext authentication
        return UserInDB(username="admin", hashed_password=pwd_context.hash("admin"), roles=["Administrator"], twofa_enabled=True)
    if username == "pm" and password == "pm":
        return UserInDB(username="pm", hashed_password=pwd_context.hash("pm"), roles=["Project Manager"], twofa_enabled=False)
    return None

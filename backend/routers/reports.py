...
async def get_reports(current_user: Annotated[dict, Depends(get_current_user)]):
    ...
    if "Administrator" in user_roles or "Department Head" in user_roles:
        ...
        if managed_projects:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Project Manager role report filtering not fully implemented yet."
            )
        else:
            return {"reports": []}
...
@router.post("/reports")
async def create_report(report_data: dict, current_user: Annotated[str, Depends(has_role(["Project Manager", "Administrator", "Engineer"]))]):
    try:

...
async def get_requests(current_user: Annotated[dict, Depends(get_current_user)]):
    ...
    try:
        requests = frappe_client.get_list(
            "ServiceRequest", filters=filters, fields=["name", "title", "status", "assigned_to", "service_object"]
        )
...
async def create_request(
    request_data: dict, current_user: Annotated[str, Depends(has_role(["Project Manager", "Administrator", "Office Manager", "Client"]))]
):
    try:
...
async def update_request_status(
    request_name: str,
    status_data: dict,
    current_user: Annotated[str, Depends(has_role(["Project Manager", "Administrator", "Engineer", "Department Head"]))],
):
    try:
...
async def upload_request_attachment(
    ...
    current_user: Annotated[str, Depends(has_role(["Project Manager", "Administrator", "Engineer", "Office Manager"]))],
):
    try:
        ...
        frappe_client.update("ServiceRequest", service_request_doc.name, {"photos": service_request_doc.photos})
